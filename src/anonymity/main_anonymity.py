...
class AnonymityApp:
    def __init__(self):
        # Models: detector.onnx for bounding boxes
        self.detector = FaceDetector()          # uses models/detector.onnx
        try:
            self.recognizer = FaceRecognizer()  # uses models/mobilefacenet.onnx
        except Exception as e:
            print(f"[WARN] Recognizer failed to load: {e}. Running in detection-only mode.")
        ...
        self.enrollment = EnrollmentModule(self.detector, self.recognizer) if self.recognizer else None
    ...
    def ai_inference_loop(self):
        ...
        while not self.stop_threads:
            ...
            # --- ENROLLMENT STATE ---
            if self.state == "ENROLLING" and self.enrollment:
            ...
            # --- RECOGNITION PHASE (MobileFaceNet only when it's time and enabled) ---
            if ran_recognition and self.recognizer and galleries and detected_boxes:
